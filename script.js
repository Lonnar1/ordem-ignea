...
const pericias = [
  { nome: "Acrobacia", attr: "agilidade", soTreinada: true },
  { nome: "Adestramento", attr: "presenca", soTreinada: true },
  { nome: "Artes", attr: "presenca", soTreinada: true },
  { nome: "Atletismo", attr: "forca" },
  { nome: "Atualidades", attr: "intelecto" },
  { nome: "Ciências", attr: "intelecto", soTreinada: true },
  { nome: "Crime", attr: "agilidade", soTreinada: true },
  { nome: "Diplomacia", attr: "presenca" },
  { nome: "Enganação", attr: "presenca" },
  ...
  { nome: "Medicina", attr: "intelecto", soTreinada: true },
  { nome: "Ocultismo", attr: "intelecto", soTreinada: true },
  { nome: "Percepção", attr: "presenca" },
  { nome: "Pilotagem", attr: "agilidade", soTreinada: true },
  { nome: "Pontaria", attr: "agilidade" },
  { nome: "Profissão", attr: "intelecto", soTreinada: true },
  { nome: "Reflexos", attr: "agilidade" },
  { nome: "Religião", attr: "presenca", soTreinada: true },
  { nome: "Sobrevivência", attr: "intelecto" },
  { nome: "Tática", attr: "intelecto", soTreinada: true },
  { nome: "Tecnologia", attr: "intelecto", soTreinada: true },
...
function carregarPersonagem(index) {
  ...
  const classeOPEl = document.getElementById("classeOP");
const nexEl = document.getElementById("nex");

if (classeOPEl) {
  classeOPEl.value = p.classeOP || "combatente";
}
...
if (nexEl) {
  ...
  const peMaxOP = document.getElementById("peMaxOP");
const sanMaxOP = document.getElementById("sanMaxOP");

if (peMaxOP) peMaxOP.value = p.peMaxOP ?? 0;
if (sanMaxOP) sanMaxOP.value = p.sanMaxOP ?? 0;

  if (resistenciasEl) resistenciasEl.value = p.resistencias || "";
  if (diarioEl) diarioEl.value = p.diario || "";

  document.getElementById("agilidade").value = p.agilidade ?? 0;
  document.getElementById("forca").value = p.forca ?? 0;
  document.getElementById("intelecto").value = p.intelecto ?? 0;
  document.getElementById("presenca").value = p.presenca ?? 0;
  document.getElementById("vigor").value = p.vigor ?? 0;
  ...
  Array.isArray(p.periciasSelecionadasOP)
    ? p.periciasSelecionadasOP
    : [];
...
const TABELA_CLASSES_OP = {
  ...
  ocultista:   { pvBase: 12, pvPorNex: 2, peBase: 4,  pePorNex: 4, sanBase: 20, sanPorNex: 4 },
};

function atualizarDerivadosOP() {
  // Por enquanto os recursos da ficha são preenchidos manualmente.
  // Mantemos a função para não quebrar chamadas existentes.
}
/* ================= SALVAR ================= */
...
function salvarTudo() {
  ...
  p.classeOP = document.getElementById("classeOP")?.value || "combatente";
  p.peMaxOP = document.getElementById("peMaxOP")?.value || 0;
p.sanMaxOP = document.getElementById("sanMaxOP")?.value || 0;
  const nexValor = parseInt(document.getElementById("nex")?.value);

p.nex = Number.isNaN(nexValor)
  ? 5
  : nexValor;
...
p.nivel = document.getElementById("nivel")?.value || "";
  ...
  p.agilidade = get("agilidade");
  p.forca = get("forca");
  p.intelecto = get("intelecto");
  p.presenca = get("presenca");
  p.vigor = get("vigor");
...
p.profs = profs;
p.periciasSelecionadasOP = [...periciasSelecionadasOP];
p.saves = saves;
  p.exaustao = exaustao;
  p.morte = morte;
  p.dominio = dominio;
...
const NOMES_TREINO = ["Não treinado", "Treinado", "Veterano", "Expert"];
const BONUS_TREINO = [0, 5, 10, 15];

function atualizarTudo() {
  const atributos = {
    agilidade: get("agilidade"),
    forca: get("forca"),
    intelecto: get("intelecto"),
  ...
  const selecionada =
    periciasSelecionadasOP.includes(pericia.nome);
  ...
  div.className = `pericia ${
    selecionada
      ? "pericia-treinada"
      : "pericia-nao-treinada"
  }`;


  div.innerHTML = `
    <div class="pericia-linha-conteudo">

      <div class="pericia-esquerda">

        <div
          class="check ${selecionada ? "ativo" : ""}"
          onclick="togglePericiaSelecionadaOP('${pericia.nome}', event)"
          title="${selecionada ? "Selecionada" : "Não selecionada"}"
        ></div>
      ...
      <button
        type="button"
        class="btn-treino-pericia grau-${grau}"
        onclick="toggleProf('${pericia.nome}', event)"
        title="${NOMES_TREINO[grau]}"
      >
        ${
          grau === 0
            ? "—"
      ...
      <span class="pericia-rolagem">
        ${dados}d20${bonus > 0 ? ` +${bonus}` : ""}
      </span>
...
function togglePericiaSelecionadaOP(nome, event) {
  if (event) event.stopPropagation();

  const index = periciasSelecionadasOP.indexOf(nome);

  if (index >= 0) {
    periciasSelecionadasOP.splice(index, 1);
  } else {
    periciasSelecionadasOP.push(nome);
  }
...
function toggleProf(nome, event) {
  ...
  // 0 = não treinado | 1 = treinado | 2 = veterano | 3 = expert
  const atual = profs[nome] || 0;
  profs[nome] = atual >= 3 ? 0 : atual + 1;

  atualizarTudo();
  salvarTudo();
}

function aoSelecionarClasseOP() {
  salvarTudo();

  const classe = document.getElementById("classeOP")?.value;

  if (!classe) return;

  abrirPopupPericiasClasse();
}

function abrirPopupPericiasClasse() {
  const modal = document.getElementById("modalPericiasClasse");
  const overlay = document.getElementById("modalPericiasClasseOverlay");
  const lista = document.getElementById("listaPericiasClasse");
  ...
  pericias.forEach((pericia) => {
    const selecionada = periciasSelecionadasOP.includes(pericia.nome);

    const item = document.createElement("label");

    item.className = "pericia-selecao-item";

    item.innerHTML = `
      <input
        type="checkbox"
        data-pericia="${pericia.nome}"
        ${selecionada ? "checked" : ""}
      >

      <span class="pericia-selecao-check"></span>

      <span class="pericia-selecao-nome">
        ${pericia.nome}
      </span>
    `;
...
function fecharPopupPericiasClasse() {
  const modal = document.getElementById("modalPericiasClasse");
  const overlay = document.getElementById("modalPericiasClasseOverlay");
...
function confirmarPericiasClasse() {
  const checkboxes = document.querySelectorAll(
    '#listaPericiasClasse input[type="checkbox"]'
  );
  ...
  checkboxes.forEach((checkbox) => {
    if (checkbox.checked) {
      periciasSelecionadasOP.push(checkbox.dataset.pericia);
    }
  });
